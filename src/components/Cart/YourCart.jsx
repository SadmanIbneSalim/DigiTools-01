...
const YourCart = ({ cart,setCart }) => {
    ...
    const handleCheckoutBtn=()=>setCart([])
      ...
      <div className="max-w-300">
       ...
       (cart.map((item, index) => (
        ...
        : 
         ...
         <div>
            <ShoppingCart className="mx-auto" size={148} color="#a8a8a8" strokeWidth={1.5} />
            <h1 className="text-center text-gray-400 py-10 text-3xl">Your cart is empty ;)</h1>
         </div>
       
       }
        <div className="flex justify-between p-3 my-4">
          <p>Total:</p>
          <h4 className="text-xl font-bold">$<span>{total}</span></h4>
        </div>

        <button onClick={handleCheckoutBtn} className="btn rounded-3xl bg-gradient-to-r from-[#4F39F6] via-[#6A2CF8] to-[#9514FA] p-5 text-white w-full">
          Proceed to Checkout
        </button>
